...
> **Currently (?v=20260906bs): Dual Wear modes. Classic Flash default for SWF-only (walk PNGs unlock Smooth). Never tofu when SWF worn. Floor bob walk + hitbox emotes. sharedEvents Phase-2 documented.**
...
Bump `LOGO_V` in `app.js` + matching `?v=` on `index.html`. See [STATUS.md](./STATUS.md). **This build:** `?v=20260906bs` — Flash/Ruffle loft playability (never tofu, floor walk, emotes, second SWF) + br club preserved (see `STATUS.md` / `HOW-CLASSIC-AVATARS-WITHOUT-FLASH.md`).
