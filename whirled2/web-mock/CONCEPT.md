...
- **Profile skins** (MySpace vibe): own profile → Look & background — color / gradient / image, accent, translucent panels, optional motto under status. Stored as `whirled2.profileSkin.{userId}`. Visitors see the skin on other profiles. **No profile music** — room playlists already cover audio. ENGINE DEV: profile chrome ≠ `#stage-slot`.
