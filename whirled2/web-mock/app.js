...
(function () {
  ...
  function classicRuffleWearHtml(worn, posStyle) {
    // (?v=20260906ca): Always mount Ruffle companion host + hitbox/nameplate + stand thumb OR glyph.
  ...
  function chromeWalkTo(xPct, yPct) {
    ...
    // (?v=20260906ca): notifyLoftWalk(true) at START keeps loftHostState.moving until arrive → hostWalk(false).
    ...
    // (?v=20260906ca): ALWAYS notify classic module on floor walk when present (Wear + hostWalk).
    ...
    try {
      // (?v=20260906ca): floor click wakes hostSleep; notifyLoftWalk(true) until arrive → hostWalk(false).
  ...
  // (?v=20260906ca): touchActivity also noteLoftActivity → hostSleep(false); Flash sleep ~60s in classic-avatar.
  ...
  function touchActivity() {
    ...
    // (?v=20260906ca): wake Flash hostSleep (~60s idle → appearance sleeping).
    try {
      if (window.WhirledClassicAvatar && WhirledClassicAvatar.noteLoftActivity) {
        WhirledClassicAvatar.noteLoftActivity();
      }
  ...
  function exposeBridge() {
    ...
    window.WhirledChrome = {
      ...
      // (?v=20260906ca): avatar setLocation_v1 bridge → chromeWalkTo (logical x*100 → %).
  ...
  async function pushChat(text) {
    ...
    // (?v=20260906ca): loft chat → hostSpoke → avatarSpoke_v1 (talk anim when companion connected).
    try {
  ...
  // (?v=20260906ca): Guest Flash QA loft WITHOUT login — ?flashQa=1 (and/or avatarDebug=1).
  ...
  function boot() {
    ...
    // (?v=20260906ca): ?flashQa=1 / avatarDebug=1 → ephemeral session before gate (browser QA).
