...
(function () {
  ...
  function classicRuffleWearHtml(worn, posStyle) {
    // (?v=20260906bz): Always mount Ruffle companion host + hitbox/nameplate + stand thumb OR glyph.
  ...
  function chromeWalkTo(xPct, yPct) {
    ...
    // (?v=20260906bz): notifyLoftWalk(true) at START keeps loftHostState.moving until arrive → hostWalk(false).
    ...
    // (?v=20260906bz): ALWAYS notify classic module on floor walk when present (Wear + hostWalk).
    ...
    try {
      // (?v=20260906bz): floor click wakes hostSleep; notifyLoftWalk(true) until arrive → hostWalk(false).
  ...
  // (?v=20260906bz): touchActivity also noteLoftActivity → hostSleep(false); Flash sleep ~60s in classic-avatar.
  ...
  function touchActivity() {
    ...
    // (?v=20260906bz): wake Flash hostSleep (~60s idle → appearance sleeping).
    try {
      if (window.WhirledClassicAvatar && WhirledClassicAvatar.noteLoftActivity) {
        WhirledClassicAvatar.noteLoftActivity();
      }
  ...
  function exposeBridge() {
    ...
    window.WhirledChrome = {
      ...
      // (?v=20260906bz): avatar setLocation_v1 bridge → chromeWalkTo (logical x*100 → %).
  ...
  async function pushChat(text) {
    ...
    // (?v=20260906bz): loft chat → hostSpoke → avatarSpoke_v1 (talk anim when companion connected).
    try {
  ...
  // (?v=20260906bz): Guest Flash QA loft WITHOUT login — ?flashQa=1 (and/or avatarDebug=1).
  ...
  function boot() {
    ...
    // (?v=20260906bz): ?flashQa=1 / avatarDebug=1 → ephemeral session before gate (browser QA).
